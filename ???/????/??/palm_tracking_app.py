...
class PalmTrackingApp:
    def __init__(self) -> None:
        ...
        self.detail_text = tk.StringVar(value="Place your palm in the center of the camera view")
        self.button_text = tk.StringVar(value="Start Tracking")
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(top, text="Palm Tracking", bg="#1c2228", fg="#f5f7fa", font=("Microsoft YaHei", 16, "bold")).pack(side=tk.LEFT, padx=(16, 18))
        ...
        tk.Label(side, text="Tracking Controls", bg="#181d22", fg="#f1f5f9", font=("Microsoft YaHei", 13, "bold")).pack(anchor="w", padx=16, pady=(18, 12))
        ...
        tk.Button(side, text="Exit", command=self.close, bg="#263039", fg="#e7edf3", activebackground="#394651", activeforeground="#ffffff", relief=tk.FLAT, font=("Microsoft YaHei", 11), padx=12, pady=9).pack(fill=tk.X, padx=16)
        tk.Label(side, text=f"Maximum speed: {self.controller.config.max_degrees_per_second:g}°/s\nStops adjusting inside the center zone\nPauses after the palm is lost for 0.5 seconds", bg="#181d22", fg="#8393a4", justify=tk.LEFT, font=("Microsoft YaHei", 10)).pack(anchor="w", padx=16, pady=(26, 0))
        ...
        while self.running:
            if self.cap is None or not self.cap.isOpened():
                ...
                if not self.cap.isOpened():
                    ...
                    time.sleep(1.5)
                    continue
                self._set_status(f"Opened camera {CAMERA_DEVICE}")
        ...
        while self.running:
            ...
            if frame is not None:
                try:
                    ...
                    self._set_status(f"Palm detection error: {exc}")
    ...
    def start_tracking(self) -> bool:
        ...
        self.button_text.set("Stop Tracking")
    ...
    def stop_tracking(self, reason: str) -> None:
        ...
        self.button_text.set("Start Tracking")
        self._set_status("Tracking stopped" if reason == "user" else reason)
        ...
        if self.running and self.tracking_enabled:
            ...
            if box is not None:
                ...
                if decision.state == "lost":
                    self._set_status("No hand detected; waiting for it to reappear")
                    ...
                    if not ok:
                        self.stop_tracking(f"Gimbal communication failed: {detail}")
                    else:
                        self._set_status("Tracking")
                elif decision.state == "centered":
                    self._set_status("Palm centered")
        if self.running:
            self.root.after(CONTROL_INTERVAL_MS, self._control_tick)
    ...
    def _update_view(self) -> None:
        ...
        if frame is None:
            ...
            if not self.tracking_enabled and self.start_button is not None:
                ...
                self.detail_text.set("Palm detected. Select Start Tracking." if ready else "Place a hand in view")
            elif self.tracking_enabled:
                self.detail_text.set("The palm box will stay near the center")
        if self.running:
