...
MIN_RECORD_SEC = 0.42
MAX_RECORD_SEC = 2.8
POST_SILENCE_SEC = 0.46
NOISE_CALIBRATE_SEC = 0.35
NOISE_TRIGGER_MULT = 1.35
NOISE_SILENCE_MULT = 1.08
MAX_DYNAMIC_TRIGGER = 0.120
MAX_DYNAMIC_SILENCE = 0.070
...
class NurseryRhymePlayer:
    ...
    def _make_tts(self, text: str) -> str | None:
        ...
        try:
            ...
            return normalize_playback_wav(path, gain=1.0, target_rate=48000, target_channels=2)
        except Exception:
            with suppress(FileNotFoundError):
                os.remove(path)
            return None
    ...
    def _build_play_file(self, name: str) -> str | None:
        ...
        try:
            ...
            if not Path(wav_path).exists() or Path(wav_path).stat().st_size <= 1024:
                return None
            return normalize_playback_wav(wav_path, gain=1.0, target_rate=48000, target_channels=2)
        except Exception:
            with suppress(FileNotFoundError):
                os.remove(wav_path)
            return None
    ...
    def _play_wav_blocking(self, path: str):
        ...
        self._log(f"play_end rc={self.play_proc.returncode} sec={elapsed:.3f} err={err!r}")
        self.play_proc = None
    ...
    def _calibrate_noise(self, pcm) -> None:
        ...
        self.dynamic_trigger = max(TRIGGER_PEAK, min(MAX_DYNAMIC_TRIGGER, noise * NOISE_TRIGGER_MULT + 0.012))
        self.dynamic_silence = max(SILENCE_PEAK, min(MAX_DYNAMIC_SILENCE, self.dynamic_trigger * 0.70, noise * NOISE_SILENCE_MULT + 0.006))
