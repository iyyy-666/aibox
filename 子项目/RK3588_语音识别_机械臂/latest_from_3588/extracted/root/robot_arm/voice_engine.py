...
VOICE_BACKEND = os.getenv("VOICE_BACKEND", "paraformer").strip().lower()
...
def extract_command(text: str, commands: tuple[str, ...]) -> str | None:
    """Extract one registered action from a natural-language ASR result."""
    if not text or not commands:
        return None
    compact = _normalize_text(text)
    if not compact:
        return None
    for command in commands:
        aliases = (command,) + tuple(HOTWORDS.get("robot", {}).get(command, ())) + tuple(COMMAND_ALIASES.get(command, ()))
...
def should_auto_upright(*, last_command_age: float, moving: bool, already_upright: bool, timeout: float = 20.0) -> bool:
    return last_command_age >= timeout and not moving and not already_upright


def idle_timeout_due(idle_sec: float, *, moving: bool, already_upright: bool, timeout: float = 20.0) -> bool:
    return idle_sec >= timeout and not moving and not already_upright
...
class VoiceEngine:
    ...
    def _review_asr(self, audio: bytes, primary: str) -> str:
        ...
        if self.commands:
            ...
            command_matched = bool(extract_command(primary, commands))
    ...
    def _match(self, text):
        ...
        matched = extract_command(text, tuple(self.commands.keys()))
    ...
    def status(self):
        if self._backend == "paraformer":
            model_name = Path(PARAFORMER_MODEL_DIR).name
        elif self._backend == "sherpa":
