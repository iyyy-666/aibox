...
TRIGGER_PEAK = float(os.getenv("AI_TRIGGER_PEAK", "0.075"))
BARGE_IN_TRIGGER_PEAK = float(os.getenv("AI_BARGE_IN_TRIGGER_PEAK", "0.140"))
BARGE_IN_ENABLED = os.getenv("AI_BARGE_IN_ENABLED", "1").strip().lower() in {"1", "true", "yes", "on"}
SILENCE_PEAK = float(os.getenv("AI_SILENCE_PEAK", "0.040"))
MIN_RECORD_SEC = float(os.getenv("AI_MIN_RECORD_SEC", "0.28"))
MAX_RECORD_SEC = float(os.getenv("AI_MAX_RECORD_SEC", "5.2"))
POST_SILENCE_SEC = float(os.getenv("AI_POST_SILENCE_SEC", "0.44"))
FAST_POST_SILENCE_SEC = float(os.getenv("AI_FAST_POST_SILENCE_SEC", "0.36"))
...
NOISE_TRIGGER_MULT = float(os.getenv("AI_NOISE_TRIGGER_MULT", "2.4"))
NOISE_SILENCE_MULT = float(os.getenv("AI_NOISE_SILENCE_MULT", "1.45"))
...
class AIAssistant:
    ...
    def _speak_sherpa_tts(self, turn: int, text: str) -> bool:
        ...
        try:
            ...
            return self._run_playback(turn, wav_path)
        except Exception as exc:
    ...
    def _calibrate_noise(self, inp) -> None:
        ...
        if not peaks:
            return
        noise = float(np.percentile(np.array(peaks, dtype=np.float32), 50))
        self._noise_peak = noise
        self._dynamic_trigger = max(TRIGGER_PEAK, min(0.260, noise * NOISE_TRIGGER_MULT + 0.014))
        self._dynamic_silence = max(SILENCE_PEAK, min(0.135, self._dynamic_trigger * 0.70, noise * NOISE_SILENCE_MULT + 0.010))
        timing_log(f"noise peak90={noise:.3f} trigger={self._dynamic_trigger:.3f} silence={self._dynamic_silence:.3f}")
        ...
        while self.running:
            ...
            if frame_peak >= trigger_level:
                ...
                if frame_peak < self._dynamic_silence:
                    ...
                    silence_samples = 0
                fast_short_done = captured_samples <= int(0.8 * SAMPLE_RATE) and silence_samples >= fast_post_silence_samples
