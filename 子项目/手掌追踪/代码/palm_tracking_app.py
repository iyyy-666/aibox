...
DIRECTION_CONFIG = os.getenv("PALM_TRACK_DIRECTION_CONFIG", os.path.expanduser("~/.config/aibox/palm_tracking_direction.json"))
...
class PalmTrackingApp:
    def __init__(self) -> None:
        ...
        saved_direction = PalmTrackingController.load_direction_config(DIRECTION_CONFIG)
        ...
        self._calibration_before: float | None = None
    ...
    def start_direction_calibration(self) -> None:
        if self.tracking_enabled or self._calibration_axis is not None or self.current_box is None:
            return
        box = self.current_box
        self._calibration_axis = "yaw"
        self._calibration_before = box[0] + box[2] / 2.0
        self._set_status("正在校准水平轴方向")
        ok, detail = self.gimbal.move(8, 0, 500)
        if not ok:
            self._calibration_axis = None
            self._set_status(f"方向校准失败：{detail}")
            return
        self.root.after(700, self._finish_yaw_calibration)

    def _finish_yaw_calibration(self) -> None:
        if self._calibration_axis != "yaw" or self._calibration_before is None or self.current_box is None:
            return
        after = self.current_box[0] + self.current_box[2] / 2.0
        self.controller.infer_axis_sign_from_motion(axis="yaw", command_delta=8, before_position=self._calibration_before, after_position=after)
        self._calibration_axis = "pitch"
        self._calibration_before = self.current_box[1] + self.current_box[3] / 2.0
        self._set_status("正在校准俯仰轴方向")
        ok, detail = self.gimbal.move(0, 8, 500)
        if not ok:
            self._calibration_axis = None
            self._set_status(f"方向校准失败：{detail}")
            return
        self.root.after(700, self._finish_pitch_calibration)

    def _finish_pitch_calibration(self) -> None:
        if self._calibration_axis != "pitch" or self._calibration_before is None or self.current_box is None:
            return
        after = self.current_box[1] + self.current_box[3] / 2.0
        self.controller.infer_axis_sign_from_motion(axis="pitch", command_delta=8, before_position=self._calibration_before, after_position=after)
    ...
    def _update_view(self) -> None:
        ...
        if frame is None:
            ...
            if not self.tracking_enabled and self.start_button is not None:
                ...
                self.detail_text.set("手掌框将保持在画面中心")
        if self.running:
